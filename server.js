...
app.get('/lowRating', function(req, res){
...
app.get('/fictionBooks', function(req, res){
...
// not Working!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
app.get('/tallReach', function(req, res){
  Person.find( {$and: [  {  height: {"$gt": 180} }, {salary: {"$gt": 30000}}]}, function(err, reachPeople){
    res.send(reachPeople)
  } )
})

//2. Find all the people who are tall (>180) OR rich (>30000)
app.get('/tallOrReach', function(req, res){
...
app.get('/beautiful', function(req, res){
  Person.find({}).and([ {$or:[ {hair: "grey" }, {eyes: "grey" } ] }, {weight: {"$lt": 70}   }]).exec(function(err, people){
    console.log(people);
    res.send(people)
  })
})
